 ...
 */
function core00_profile_image_default_styles() {
/*
  $styles = array();
 ...
 */
function core00_profile_form_install_configure_form_alter(&$form, $form_state) {
  ...
  $form['admin_account']['account']['name']['#default_value'] = 'core44';
  ...
  $form['admin_account']['account']['core00_profile_name'] = array(
    '#type' => 'item', '#title' => st('Username'),
    '#markup' => 'admin'
  );
  $form['admin_account']['account']['core00_profile_password'] = array(
    '#type' => 'item', '#title' => st('Password'),
    '#markup' => 'admin'
  );
  $form['admin_account']['account']['core00_profile_informations'] = array(
  ...
  array_unshift($form['#validate'], 'core00_profile_custom_setting');
}
 ...
 * custom values.
 */
function core00_profile_custom_setting(&$form, &$form_state) {
 ...
 */
function core00_profile_system_info_alter(&$info, $file, $type) {
 ...
 */
function core00_profile_update_projects_alter(&$projects) {
/*
 ...
 */
function core00_profile_update_status_alter(&$projects) {
  $bad_statuses = array(
    UPDATE_NOT_SECURE,
    UPDATE_REVOKED,
  ...
  $make_filepath = drupal_get_path('module', 'core00_profile') . '/drupal-org.make';
...
//    if (strpos($project_name, 'core00_profile_') !== FALSE) {
 ...
 */
function core00_profile_form_update_manager_update_form_alter(&$form, &$form_state, $form_id) {
  if (isset($form['projects']['#options']) && isset($form['projects']['#options']['core00_profile'])) {
    if (count($form['projects']['#options']) > 1) {
      unset($form['projects']['#options']['core00_profile']);
    }
    else {
      unset($form['projects']);
 ...
 */
function core00_profile_crumbs_get_info() {
  $crumbs = array(
    'crumbs.home_title' => 0
  );

  foreach (module_implements('core00_profile_crumb_info') as $module) {
    // The module-provided item might be just the name of the plugin, or it
    // might be an array in the form of $plugin_name => $weight.
    foreach (module_invoke($module, 'core00_profile_crumb_info') as $crumb) {
      if (is_array($crumb)) {
        $crumbs += $crumb;
      }
      else {

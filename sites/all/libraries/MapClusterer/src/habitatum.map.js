 ...
 */
 ...
 var mapboxUrl, mapboxAttrib, mapbox;
...
function HabitatumMap(element, api_key, opt_markers, cluster_handlers) {
  ...
  mapboxUrl = 'https://{s}.tiles.mapbox.com/v3/filthypixel.gocl3h9d/{z}/{x}/{y}.png';
  ...
  var london = new L.LatLng(0, 0);
  map.setView(london, 3).addLayer(mapbox);
...
function HabitatumCluster(habitatumMap, eventHandlers) {
  ...
  this.createClusterMarker = function(latlng, text, padding){
    ...
    var style = { className: "dfhdh", color: '#fff', fillColor: '#f03', fillOpacity: 0.5, radius: 20, label: text };
    var  marker = new L.CircleMarkerEx(latlng, style);
...
L.CircleMarkerEx = L.CircleMarker.extend({
  ...
  _initPath: function() {
		...
		if (this._label !== null){
			...
			this._text.setAttribute('style', 'writing-mode:lr-tb; line-height:125%; text-align:center; text-anchor: middle; dominant-baseline: central; font-weight:normal;');

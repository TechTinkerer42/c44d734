 ...
 */
 ...
 var mapboxUrl, mapboxAttrib, mapboxTiles;
 ...
 */
function scuboxMap(element, mapboxID, opt_markers, cluster_handlers) {
  var dis = this;
...
/*
  ...
  mapboxUrl = 'https://{s}.tiles.mapbox.com/v3/'+mapboxID+'/{z}/{x}/{y}.png';
	mapboxAttrib = '<a href="http://www.mapbox.com/about/maps/" target="_blank">Terms &amp; Feedback</a>';
	mapboxTiles = new L.TileLayer(
				mapboxUrl, {
						maxZoom: 18, 
  ...
  //map.setView([0,0], 3).addLayer(mapbox);
...
*/
  ...
  this.addMarker = function (marker, opt_isNodraw, opt_isAdded, opt_clusters, opt_isNoCheck) {
    ...
    cluster = new scuboxCluster(this, clusterEventHandlers);
    cluster.addMarker({
      'isAdded': isAdded,
      'marker': marker
    });
  ...
  this.resetViewport = function () {
    ...
    for (var i = 0; i < clusters.length; ++i) {
      var scuboxCluster = clusters[i];
      var oldZoom = scuboxCluster.getCurrentZoom();
      if (oldZoom === null) {
        continue;
      }
      ...
      if (curZoom !== oldZoom) {
        ...
        // If the scuboxCluster zoom level changed then destroy the scuboxCluster
        // and collect its markers.
        var mks = scuboxCluster.getMarkers();
        ...
        scuboxCluster.clearMarkers();
        removed++;
        for (j = 0; j < clusters_.length; ++j) {
          if (scuboxCluster === clusters_[j]) {
    ...
    // Add the markers collected into marker scuboxCluster to reset
 ...
 * @constructor
 * @private
 * @param {scuboxMap} scuboxMap The marker cluster object
 */
function scuboxCluster(scuboxMap, eventHandlers) {
  var dis = this;
  ...
  var markers_ = [];
  var scuboxMap_ = scuboxMap;
  var map = scuboxMap.getmap();
  ...
  this.isInBounds = function (bounds) {
    ...
    var inViewport = true;
    var gridSize = scuboxMap_.getGridSize_();
  ...
  this.redraw = function (isForce) {
    ...
    if (zoom_ >= mz || this.getTotalMarkers() === 1) {
      ...
      if (clusterMarker === null) {
        clusterMarker = this.createClusterMarker(center_, this.getTotalMarkers(), scuboxMap_.getGridSize_());
...
function scuboxMarker(latlng, text, eventHandlers){
	this.map_ = map;
	this.latlng_ = latlng;

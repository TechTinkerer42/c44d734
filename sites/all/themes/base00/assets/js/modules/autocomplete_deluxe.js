...
(function($) {
  ...
  Drupal.autocomplete_deluxe.Widget.prototype.init = function(settings) {
    ...
    var generateValues = function(data, term) {
      ...
      if ($.isEmptyObject(result)) {
        result.push({
          label: Drupal.t("Click to add '@term'", {'@term' : term}),
          value: term,
          newTerm: true
        });
      }
  ...
  Drupal.autocomplete_deluxe.MultipleWidget.Item = function (widget, item) {
    ...
    var self = this;
    //EDITED HERE
    var close = $('<span class="autocomplete-deluxe-item-remove glyphicon glyphicon-remove"></span>').appendTo(this.element);
  ...
  Drupal.autocomplete_deluxe.MultipleWidget.prototype.setup = function() {
    ...
    // all the time.
    var autocompleteDataKey = typeof jqObject.data('autocomplete') == 'object' ? 'autocomplete' : 'ui-autocomplete';
